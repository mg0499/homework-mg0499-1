...
def poland_cases_by_date(day: int, month: int, year: int = 2020) -> int:
    ...
    year2=year%100
    result = confirmed_cases.loc[confirmed_cases["Country/Region"]=="Poland"][f"{month}/{day}/{year2}"].values[0]
    return result
...
def top5_countries_by_date(day: int, month: int, year: int = 2020) -> List[str]:
    ...
    retList = cs_sorted[["Country/Region", date]].sort_values(by=date)["Country/Region"].tail(5).tolist()
    retList.reverse()
    #print(retList)
    return retList
...
def no_new_cases_count(day: int, month: int, year: int = 2020) -> int:
    ...
    #cc = confirmed_cases.groupby(["Country/Region"], as_index = False).sum();
    cc=confirmed_cases
